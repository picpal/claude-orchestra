...
14개 전문 에이전트 기반 TDD 개발 오케스트레이션 시스템
...
Claude Orchestra는 14개의 전문 에이전트가 계층 구조로 협력하여 TDD(Test-Driven Development) 기반의 고품질 코드를 생성하는 오케스트레이션 시스템입니다.
...
| 복사 대상 | 목적지 | 이유 |
|-----------|--------|------|
| `rules/*.md` | `.claude/rules/` | **프로젝트별 규칙 커스터마이징** - 보안, 테스트, 코딩 스타일 등 규칙을 프로젝트 특성에 맞게 수정 가능 |
| (생성) | `.orchestra/` | **프로젝트별 상태 관리** - 계획, 작업 일지, 로그 등 프로젝트 고유 데이터 저장 |

**왜 플러그인에서 직접 실행하지 않나요?**

1. **격리성**: 각 프로젝트는 독립적인 상태(`.orchestra/`)를 가져야 합니다
2. **커스터마이징**: `rules/`를 프로젝트에 복사하면 팀/프로젝트별로 규칙 수정 가능
3. **버전 관리**: 프로젝트의 `.claude/rules/`를 git에 커밋하여 팀원과 규칙 공유
4. **충돌 방지**: 여러 프로젝트가 동시에 실행되어도 상태가 섞이지 않음

```
플러그인 (읽기 전용)          프로젝트 (읽기/쓰기)
├── rules/          ──복사──▶  .claude/rules/  (수정 가능)
├── agents/         ◀──참조──
├── hooks/          ◀──참조──
