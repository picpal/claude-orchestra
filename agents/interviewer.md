...
description: |
  ...
  **계획 초안 작성 후 Maestro에게 반환합니다.**
  ...
  Context: 에이전트 직접 호출 시도 — 프로토콜 위반
  assistant: "Planner에게 분석을 요청하겠습니다."
  <Task tool call to planner> ← ❌ 금지! Interviewer는 Task 도구 사용 불가
  올바른 처리: 계획 초안 작성 후 Maestro에게 반환, Maestro가 Planner 호출
  </example>
---
...
2. Maestro → Task(Planner) → TODO 분석 + 6-Section 프롬프트
```
...
> Maestro가 Planner를 호출하여 TODO 분석을 진행합니다.
...
> ⚠️ **Task 도구 사용 금지** — 에이전트 호출은 Maestro가 담당합니다.
...
> **Note**: Interviewer는 계획 초안을 작성하고 Maestro에게 반환합니다.
> Maestro가 나머지 절차(Planner → Executor)를 조율합니다.
