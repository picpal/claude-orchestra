...
description: |
  ...
  **계획 초안 작성 후 Maestro에게 반환합니다. Plan Validation Group 호출은 Maestro가 담당합니다.**
...
**Plan Validation Group 호출은 Maestro가 담당합니다.**
...
│  3. Maestro에게 반환                                            │
│                                                                 │
│  ⚠️ Plan Validation Group 호출은 Maestro가 담당                 │
└─────────────────────────────────────────────────────────────────┘

전체 흐름 (Maestro 관점):
1. Maestro → Task(Interviewer) → 계획 초안 반환
2. Maestro → Task(Plan Validation Group) → 분석 + 검증 → 승인/거부
...
3. **Maestro에게 결과 반환** (Plan Validation Group는 Maestro가 호출)
...
[Phase 4: Maestro에게 반환]
    - 계획 초안 완료 보고
    - Plan Validation Group 검토 필요 표시
```
...
- TODOs: {N}개
- Groups: {group-list}
- Plan Validation Group 검토 필요
```

> ⚠️ **중요**: 이 출력은 "계획 초안"입니다. 최종 승인이 아닙니다.
> Maestro가 Plan Validation Group를 호출하여 검토를 진행합니다.
...
│                                                                 │
│  🚫 Plan Validation Group 호출은 Maestro가 담당                 │
└─────────────────────────────────────────────────────────────────┘
```
...
> ⚠️ **Task 도구 사용 금지** — Plan Validation Group 호출은 Maestro가 담당합니다.
...
- Plan Validation Group 직접 호출 (Task 도구 사용 금지)
...
> **Note**: 이전 버전과 달리, Interviewer는 Plan Validation Group를 호출하지 않습니다.
