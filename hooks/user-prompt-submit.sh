...
# Plan Mode 통합 감지
# Plan Mode 종료 후 interviewerCompleted가 false이면 안내 메시지 출력
PLAN_MODE_HINT=""
if [ -f "$STATE_FILE" ]; then
...
# Session Journal 소프트 힌트 (activity/changes 로그 존재 시)
if [ "$JOURNAL_REQUIRED" != "true" ] && [ "$MODE" = "IDLE" ]; then
  ACTIVITY_LOG="$ORCHESTRA_LOG_DIR/activity.jsonl"
  CHANGES_LOG="$ORCHESTRA_LOG_DIR/changes.jsonl"
  if [ -f "$ACTIVITY_LOG" ] || [ -f "$CHANGES_LOG" ]; then
    cat <<'SESSION_HINT'

> **[참고]** 이번 세션에서 활동/변경 로그가 기록되었습니다. 세션을 마칠 때 Session Journal 작성을 권장합니다.
> 파일: `.orchestra/journal/session-{YYYYMMDD}-{HHmm}.md` (maestro-protocol.md Session Journal Template 참조)
...
- 모든 TODO가 완료되고 Verification + Git Commit까지 끝났다면:
  1. \`.orchestra/journal/{plan-name}-{YYYYMMDD}-{HHmm}.md\` 파일을 작성하세요
  2. **maestro-protocol.md Phase 7의 OPEN-ENDED Journal Template** 양식을 준수하세요
  3. journal-tracker.sh가 자동으로 state.json을 업데이트합니다 (수동 변경 불필요)
